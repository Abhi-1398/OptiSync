    ...
    const validateData = (name,value) => {
        ...
        switch(name){
            case 'name':
                ...
                {
                    hasError = true;
                    error = "maximum length should be 20"
                }
                break;
            case 'person_name':
                var exp1 = /^[a-z]{5,20}$/
...
const sendData = (e) => {
    ...
    const reqOption = {
        ...
        body: JSON.stringify({
            ...
            mode_of_transaction:info.mode_of_transaction.value
        })
    }
...
const[price,setprice]=useState();
 const p=allPlan.map(pl=>{ if(pl.id==planid){return pl.price}});
 console.log(p);
